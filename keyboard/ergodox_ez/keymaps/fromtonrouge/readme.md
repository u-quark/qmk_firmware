...
A stroke is generated by evaluating the following rules in this order :

	CASE_CONTROLS -> LEFT_USER_SYMBOLS -> LEFT_HAND -> THUMBS -> RIGHT_HAND -> RIGHT_PINKY -> LEFT_NUMBERS -> RIGHT_NUMBERS -> RIGHT_USER_SYMBOLS -> SPACES_AND_OTHERS	
	
	Blabla (TODO)

In this custom layout you can find some C++ operators and punctuations and also a specific Vim key (ESC:)

Numbers are binary numbers (experimental) : you can stroke 2 numbers at the same time (from 0 to 15 for each hand)
...
### Lookup table for left user symbols (C++ symbols in this example)
...
### Lookup table for right user symbols (C++ symbols in this example)
